...
$script:GHBase    = 'https://raw.githubusercontent.com/Zero1779/Collect-Free/main/apps'
$script:GHRelease = 'https://github.com/Zero1779/Collect-Free/releases/download'
...
function Invoke-BootstrapDownloads {
    ...
    $items = @(
        ...
        @{ W = '';                                 F = 'Collect_AMD_Free_Guia.html';    R = '' },
...
function Invoke-GPUOpt {
    ...
    # Pega GPU dedicada — filtra iGPU e Microsoft Basic Display Adapter
...
function Invoke-ImportNvidiaProfile {
    ...
    $npiFolder = if (Test-Path 'C:\Collect\nvidiaInspector') {
        'C:\Collect\nvidiaInspector'
    } elseif (Test-Path 'C:\Collect\nvidiaProfileInspector') {
        'C:\Collect\nvidiaProfileInspector'
    } else { '' }
    ...
    while ($running) {
        ...
        switch ($choice) {
            ...
            'A' {
                ...
                Write-Host ''
                Write-Center 'Criando ponto de restauracao antes de iniciar...' 'DarkGray'
                Invoke-RestorePoint
                Write-Host ''
                ...
                Write-Center 'Tudo aplicado com sucesso!' 'Green'
                Write-Center 'Reinicie o PC para que todas as alteracoes entrem em vigor.' 'Yellow'
                Write-Host ''
